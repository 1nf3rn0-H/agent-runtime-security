...
Agent Runtime Security receives multiple hook observations for one tool lifecycle. Pre- and post-tool events expose the same Codex `tool_use_id`, while permission-request events do not document that field. A single identifier cannot honestly represent both exact and inferred correlation.
...
When `tool_use_id` is present, Agent Runtime Security derives `action_id` from the trace and tool-use identifier, producing an exact pre/post lifecycle join. Otherwise it derives the action identifier from the request fingerprint and labels the correlation `best_effort`.
...
- Allowed shell processes inherit `ARS_ACTION_ID` and `ARS_REQUEST_FINGERPRINT`, so process sensors can join observations without translating the vendor tool-call ID.
